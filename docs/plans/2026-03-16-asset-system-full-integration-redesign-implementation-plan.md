...
| 批次 2 | Task 2 收敛前端共享类型和 API 契约 | 进行中 | 先补失败测试，再统一 `AssetRef` 与业务单据基础契约 |
| 批次 3 | Task 3 收敛后端 Controller、DTO 和状态回写基础契约 | 待开始 | 对齐入参与响应字段，统一后端基础口径 |
| 批次 4 | Task 4 完成资产分类与动态属性建模收口 | 待开始 | 收口分类树、属性元数据与保留字段校验 |
| 批次 5 | Task 5 完成资产台账聚合表单与财务附件收口 | 待开始 | 收口聚合详情、动态属性、附件与财务日志 |
